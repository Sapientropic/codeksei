...
test("SystemMessageQueueStore infers legacy metadata and only drains ready messages", () => {
  ...
  assert.deepEqual(ready.map((entry) => entry.id), ["legacy-checkin"]);
  ...
  assert.deepEqual(futureReady.map((entry) => entry.id), ["reminder:future"]);
...
test("SystemMessageQueueStore keeps leased ready messages recoverable across a restart", () => {
  ...
  assert.deepEqual(retried.map((entry) => entry.id), ["leased-message"]);
});
...
test("SystemMessageQueueStore records dead-letter before removing the live queue entry", () => {
  ...
  assert.equal(recoveredQueue.listDeadLetters().some((entry) => entry.id === "dead-letter-order"), true);
