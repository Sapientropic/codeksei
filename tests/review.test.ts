...
interface ReviewFixtureConfig {
  diaryDir: string;
  reviewSchemaConfigFile: string;
  reviewSemanticGenerator?: ((input: { profile: { kind: string } }) => Promise<unknown>) | null;
  reviewSemanticMode: string;
  workspaceRoot: string;
}

function setupReviewFixture(): {
  config: ReviewFixtureConfig;
  diaryDir: string;
  workspaceRoot: string;
} {
...
test("review hybrid v2 lets semantic pass replace noisy deterministic lines", async () => {
  ...
  fixture.config.reviewSemanticGenerator = async ({ profile }: { profile: { kind: string } }) => {
