...
test("root command flags do not masquerade as subcommands", () => {
  ...
  assert.equal(resolveTerminalCommandManifest("host", "manifest")?.action, "host.manifest");
  assert.equal(resolveTerminalCommandManifest(["host", "claim-checkin", "--provider", "hermes"])?.action, "host.claim_checkin");
  ...
  assert.equal(resolveTerminalCommandManifest(["operator", "hermes", "sync-checkin", "--user", "wxid_xxx"]), null);
});
