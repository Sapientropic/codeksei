...
function normalizeSystemMessageDeadLetterEntry(entry) {
...
function normalizeTimelineScreenshotJob(job) {
  ...
  const args = Array.isArray(job.args)
    ? job.args.map((value) => normalizeText(value)).filter(Boolean)
    : [];
...
function compareSystemMessageDeadLetters(left, right) {
...
function clearSystemMessageInFlight(message) {
...
function markSystemMessageInFlight(message, nowMs = Date.now()) {
  return normalizeSystemMessage({
    ...message,
...
function isSystemMessageExpired(message, nowMs = Date.now()) {
