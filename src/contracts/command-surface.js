...
const COMMAND_ACTIONS_BY_ID = new Map(COMMAND_ACTIONS.map((entry) => [entry.action, entry]));
...
const TERMINAL_COMMAND_MANIFEST_BY_SCRIPT_NAME = new Map(
  TERMINAL_COMMAND_MANIFEST
    ...
    .map((entry) => [normalizeText(entry.scriptName), entry])
);
...
function findCommandAction(action) {
...
function findTerminalManifestByScriptName(scriptName) {
