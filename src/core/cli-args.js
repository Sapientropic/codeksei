...
function sliceLeafCommandArgs(argv = process.argv, startIndex = 4) {
...
function parseCliArgs(args, schema, {
  ...
  const options = buildDefaultOptions(normalizedSchema);
...
function buildDefaultOptions(schema) {
  const options = {};
...
function shouldCapturePassthroughValue(token, next) {
