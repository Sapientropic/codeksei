...
function createAppServices({
  ...
  const streamDelivery = new StreamDelivery({
    ...
    onDeliveryFailure: (payload) => handleReplyDeliveryFailure(payload),
  });
  ...
  const channelCommandRouter = new ChannelCommandRouter({
    workspaceHandlers: createWorkspaceCommandHandlers({
      ...
      resolveWorkspaceRoot,
      runtimeAdapter,
      scheduleRuntimeEventWatchdog: (payload) => runtimeWatchdogLifecycle.scheduleRuntimeEventWatchdog(payload),
      streamDelivery,
      threadStateStore,
    }),
  ...
  const runtimeTurnLifecycle = new RuntimeTurnLifecycle({
    ...
    config,
    formatErrorMessage,
    maybeDispatchCommand: (normalized) => channelCommandRouter.maybeDispatchCommand(normalized),
    normalizeText,
    persistIncomingWeixinAttachments,
    queuePendingWorkspaceBootstrap: (payload) => runtimeWatchdogLifecycle.queuePendingWorkspaceBootstrap(payload),
    resolveDefaultTerminalUser,
    resolveTimelineScreenshotOutput,
    resolveWorkspaceRoot,
    runtimeAdapter,
