...
function TimelinePanel({ timeline }: TimelinePanelProps): JSX.Element {
...
function useTimelineZoom({
  ...
  const bindWheelZoom = useCallback((element: HTMLDivElement) => {
    const wheelTarget = element.querySelector(".vis-panel.vis-center") || element;
    const handleWheel = (event: Event) => {
      const wheelEvent = event as WheelEvent;
      event.preventDefault();
      const normalizedDelta = Math.abs(wheelEvent.deltaY) < 4
        ? wheelEvent.deltaY * 12
        : wheelEvent.deltaY;
      ...
      const ratio = hasUsableRect
        ? Math.min(1, Math.max(0, (wheelEvent.clientX - rect.left) / rect.width))
        : 0.5;
...
function TimelineTooltipPortal({ tooltipRef }: TimelineTooltipPortalProps): JSX.Element | null {
